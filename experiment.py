...
parser.add_argument('--DB_eps', type=float, default=5, metavar='DBE',
...
def run(args, kwargs):
    ...
    dir = snapshots_path  + '_' + model_name +  '/'
    ...
    # loading data
    train_loader, val_loader, test_loader, p_loader, args = load_dataset(args, **kwargs)
    ...
    experiment_vae(args, train_loader, val_loader, test_loader, p_loader, model, optimizer, dir, model_name = args.model_name)
    # ======================================================================================================================
    print('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-')
    with open('vae_experiment_log.txt', 'a') as f:
        print('-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n', file=f)
