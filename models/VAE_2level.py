...
class VAE(Model):
    def __init__(self, args):
        ...
        # encoder: q(z2 | z1)
        self.q_z2_layers_z1 = nn.Sequential(
            GatedDense(np.prod(self.args.z1_size), 300),
            GatedDense(300, 300)
        )
    ...
    def calculate_loss(self, x, beta=1., average=False):
        ...
        '''
        # pass through VAE
        x_mean, x_logvar, z1_q, z1_q_mean, z1_q_logvar, z2_q, z2_q_mean, z2_q_logvar, z1_p_mean, z1_p_logvar, z1_p = self.forward(x)
    ...
    def generate_x(self, N=25):
        if self.args.prior == 'standard':
            ...
            z2_sample_rand = self.q_z2Ix(means)
        
        elif self.args.prior in ['vampprior_data', 'mbap_prior', 'clust_db', 'clust_kmeans']:
            z2_sample_rand = self.q_z2Ix(self.p_loaded[0:N])

        samples_rand, _ = self.p_xIz2(z2_sample_rand)
        return samples_rand

    def reconstruct_x(self, x):
        x_mean, _, _, _, _, _, _, _, _, _, _ = self.forward(x)
        return x_mean

    # THE MODEL: VARIATIONAL POSTERIOR
    def q_z2(self, z1):
        z1 = self.q_z2_layers_z1(z1)

        z2_q_mean = self.q_z2_mean(z1)
        z2_q_logvar = self.q_z2_logvar(z1)
        return z2_q_mean, z2_q_logvar

    def q_z1(self, x):
        x = self.q_z1_layers_x(x)

        z1_q_mean = self.q_z1_mean(x)
        z1_q_logvar = self.q_z1_logvar(x)
        return z1_q_mean, z1_q_logvar

    def q_z2Ix(self, x):
        z1_sample_gen_mean, z1_sample_gen_logvar = self.q_z1(x)
        z1_sample_rand = self.reparameterize(z1_sample_gen_mean, z1_sample_gen_logvar)

        z2_sample_gen_mean, z2_sample_gen_logvar = self.q_z2(z1_sample_rand)
        z2_sample_rand = self.reparameterize(z2_sample_gen_mean, z2_sample_gen_logvar)
        return z2_sample_rand
    ...
    def p_x(self, z1):
        ...
        x_mean = self.p_x_mean(z1)
        if self.args.input_type == 'binary':
            x_logvar = 0.
        else:
            x_mean = torch.clamp(x_mean, min=0.+1./512., max=1.-1./512.)
            x_logvar = self.p_x_logvar(z1)
        return x_mean, x_logvar
    
    def p_xIz2(self, z2):
        z1_mean, z1_logvar = self.p_z1(z2)
        z1_samples = self.reparameterize(z1_mean, z1_logvar)
        x_mean, x_logvar = self.p_x(z1_samples)
        return x_mean, x_logvar

    # set prior inputs
    def set_p_data(self, X):
        p_data = X.detach().clone()
        self.p_loaded = p_data
    ...
    def log_p_z2(self, z2):
        if self.args.prior == 'standard':
            ...
            z2_p_mean, z2_p_logvar = self.q_z2(z1_sample_rand)  # C x M
