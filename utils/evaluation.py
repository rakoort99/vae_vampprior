...
from utils.visual_evaluation import plot_images, qz_pca_scatter
...
def evaluate_vae(args, model, train_loader, data_loader, epoch, dir, mode):
    ...
    if mode == 'test':
        ...
        # CALCULATE log-likelihood
        t_ll_s = time.time()
        log_likelihood_test = model.calculate_likelihood(test_data, dir, mode='test', S=args.S, MB=args.MB) #commented for testing bcz slow
        t_ll_e = time.time()
